...
{
	int datalink_value; /*unsigned char *user;  Will do nothing (create a u_char and print the size and put negative numbers to it)*/
	...
	if(argc != 2)
	{
		printf("You must enter the name of the device ! (e.g., wlan0, eth0)\n");
		exit(EXIT_SUCCESS);
	}
	...
	if(handler == NULL)
	{
		fprintf(stderr, "Error while opening the device. You may need to grant permissions to access the card in promiscuous mode (i.e., run the program again with sudo).\n");
		exit(EXIT_FAILURE);	
	}
